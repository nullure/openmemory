...
type openai_embeddings_response = {
  data: Array<{ embedding: number[]; index?: number }>
}

type openai_client = {
  embeddings: {
    create: (params: openai_embeddings_create) => Promise<openai_embeddings_response>
  }
}
...
export class OpenAIEmbeddingProvider implements EmbeddingProvider {
  private client: openai_client | null
  ...
  private async get_client(): Promise<openai_client> {
    if (this.client) return this.client
    const mod = (await import("openai")) as { default: new (opts: { apiKey: string }) => openai_client }
